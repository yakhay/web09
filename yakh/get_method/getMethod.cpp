...
void get_method(request &req, manyServer *server, int newsockfd, int idx, cl& client) {
    ...
    {    
        ...
        {
            if (req.get_path().find(client.location->_name_Location) != std::string::npos)
            {
                
                new_path = client.location->_root + req.get_path().substr(client.location->_name_Location.size(),req.get_path().size() - client.location->_name_Location.size());
                break;
            }
            client.location++;
        }
    ...
    if (client.location->_return)
    {
        ...
        if(client.location->cgi_status == 0)
        {
